...
import 'local_face_database.dart';
...
class InfoPerson {
  ...
  void update(InfoPerson other) {
    ...
    this.isRecognized = other.isRecognized;
...
class APIFace {
  ...
  StreamController streamRecognitionController = StreamController.broadcast();
  ...
  void init(RootIsolateToken rootIsolateToken) {
    ...
    camera.streamDetectFaceController.stream.listen((event) {
      if (event is List) {
                ...
                if (persons.length > 0) {
                  ...
                  for (int j = 0; j < persons.length; j++) {
                    ...
                    if (persons[j].faceId.compareTo(
                      ...
                      persons[j].angleX = faces[i].headEulerAngleX ?? 90;
                      persons[j].angleY = faces[i].headEulerAngleY ?? 90;
                      persons[j].angleZ = faces[i].headEulerAngleZ ?? 90;
                      ...
                      if (persons[j].angleX.abs() < 45 &&
                        ...
                        if (persons[j].busy == false) {
                          ...
                          if (persons[j].phone.isEmpty) {
                            ...
                            /// x1, y1: Góc trên bên trái, dựa trên ngưỡng s_x1 (0.6) và s_y1 (0.75) để thu hẹp vùng.
                            /// x2, y2: Góc dưới bên phải, dựa trên ngưỡng s_x2 (0.6) và s_y2 (0.85) để mở rộng vùng.
                            int x1 =
                                (persons[j].x - persons[j].w * s_x1).toInt();
                            int y1 =
                                (persons[j].y - persons[j].h * s_y1).toInt();
                            int x2 =
                                (persons[j].x + persons[j].w * s_x2).toInt();
                            int y2 =
                                (persons[j].y + persons[j].h * s_y2).toInt();
                  ...
                  if (flag == false) {
                    ...
                    info.angleX = faces[i].headEulerAngleX ?? 90;
                    info.angleY = faces[i].headEulerAngleY ?? 90;
                    info.angleZ = faces[i].headEulerAngleZ ?? 90;
            ...
            for (int i = 0; i < persons.length; i++) {
              if (persons[i].check == true) {
                ...
                if (time > 2000) {
                  ...
                  // Thực hiện nhận diện local
                  _performLocalRecognition(persons[i]);
  ...
  // Thực hiện nhận diện local
  Future<void> _performLocalRecognition(InfoPerson person) async {
    if (person.image.isEmpty) return;

    try {
      final result = await LocalFaceDatabase.recognizeFace(person.image);

      if (result != null) {
        person.id = result['face_id'];
        person.name = result['name'];
        person.phone = result['phone'];
        person.similarity = result['similarity'];
        person.isRecognized = true;

        app_config.printLog(
          'i',
          '[Local Recognition] Recognized: ${person.name} (${person.phone}) - Similarity: ${(person.similarity * 100).toStringAsFixed(1)}%',
        );

        // Gửi thông báo nhận diện thành công
        streamRecognitionController.add({
          'type': 'recognized',
          'person': person,
        ...
        person.similarity = 0.0;

        app_config.printLog(
          'i',
          '[Local Recognition] Unknown face: ${person.faceId}',
        );

        // Gửi thông báo khuôn mặt không nhận diện được
        streamRecognitionController.add({'type': 'unknown', 'person': person});
      }
    } catch (e) {
      app_config.printLog('e', '[Local Recognition] Error: $e');
      person.isRecognized = false;
      person.similarity = 0.0;
    }
  }
  ...
  // Đăng ký khuôn mặt mới vào database local
  Future<bool> registerNewFace(String faceId, String name, String phone) async {
    if (persons.isNotEmpty) {
      for (int i = 0; i < persons.length; i++) {
        if (persons[i].faceId.compareTo(faceId) == 0) {
          if (persons[i].image.isNotEmpty) {
            final success = await LocalFaceDatabase.registerFace(
              name: name,
              phone: phone,
              faceImage: persons[i].image,
              faceId: faceId,
            );

            if (success) {
              // Cập nhật thông tin trong persons
              persons[i].id = faceId;
              persons[i].name = name;
              persons[i].phone = phone;
              persons[i].isRecognized = true;

              app_config.printLog(
                'i',
                '[Local Registration] Successfully registered: $name ($phone)',
              );
              return true;
            }
          }
  ...
  Future<List<Map<String, dynamic>>> getAllRegisteredFaces() async {
    return await LocalFaceDatabase.getAllRegisteredFaces();
  }

  // Xóa khuôn mặt đã đăng ký
  Future<bool> deleteRegisteredFace(String faceId) async {
    return await LocalFaceDatabase.deleteFace(faceId);
  }

  // Cập nhật thông tin khuôn mặt
  Future<bool> updateRegisteredFace({
    required String faceId,
    String? name,
    String? phone,
  }) async {
    return await LocalFaceDatabase.updateFace(
      faceId: faceId,
      name: name,
      phone: phone,
    );
